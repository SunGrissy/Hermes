...
from advisors.miaomiao_advisor import get_miaomiao_advisor
...
if HAS_STREAM:
    class MiaomiaoChatbotHandler(ChatbotHandler):
        """妙妙机器人消息处理器：处理@消息"""

        def __init__(self, settings):
            super().__init__()
            self.settings = settings
            self.advisor = get_miaomiao_advisor()
            self.loop = asyncio.new_event_loop()

        async def raw_process(self, callback_message):
            """重写 raw_process 直接同步调用 process，捕获异常"""
            log_path = os.path.join(os.path.dirname(__file__), 'miaomiao_stream.log')
            try:
        ...
        def process(self, callback_message) -> AckMessage:
            ...
            try:
                ...
                print(f"[妙妙Stream] 收到消息 sender={message.sender_staff_id}")
                print(f"[妙妙Stream] 内容={message.text.content if message.text else 'None'}")
                print(f"[妙妙Stream] is_in_at_list={message.is_in_at_list}")
                ...
                print(f"[妙妙Stream] 处理查询: {user_query[:50]}")
                ...
                self.reply_text(f"[妙妙] 处理消息时出错了，请稍后再试", message)
                return AckMessage.STATUS_OK, "OK"
    ...
    def _start_stream_client(settings):
        ...
        if not settings.dingtalk_client_id or not settings.dingtalk_app_secret:
            print("[妙妙Stream] 未配置钉钉凭证，跳过Stream连接")
            return
        ...
        def run_stream():
            ...
            handler = MiaomiaoChatbotHandler(settings)
            stream_client.register_callback_handler(ChatbotMessage.TOPIC, handler)
            print("[妙妙Stream] 正在连接钉钉Stream服务器...")
            stream_client.start_forever()

        stream_thread = threading.Thread(target=run_stream, daemon=True)
        stream_thread.start()
        print("[妙妙Stream] 后台线程已启动")

else:
    def _start_stream_client(settings):
        print("[妙妙Stream] dingtalk-stream SDK未安装，跳过Stream连接")
...
async def send_daily_advice():
    """每日主动发送建议"""
    print(f"[妙妙] 执行每日建议任务 {datetime.now().isoformat()}")
    advisor = get_miaomiao_advisor()
...
async def lifespan(app: FastAPI):
    ...
    add_cron_job(send_daily_advice, hour=9, minute=0, id="miaomiao_daily")
    print("[妙妙] 定时任务已注册（每日9:00）")

    # 启动Stream连接（除非显式禁用）
    if os.environ.get("DISABLE_STREAM", "").lower() not in ("1", "true", "yes"):
        _start_stream_client(settings)
    else:
        print("[妙妙] DISABLE_STREAM=1，跳过Stream连接（由Hermes Gateway处理）")
...
app = FastAPI(title="关怀妙妙服务", lifespan=lifespan)
