...
当用户有大量文档需要 Agent 理解时，频繁开新会话会重复加载文档（浪费），但会话过长又会导致 Cache Read 暴增。平衡点如下。

### 成本结构（简化）

- **F** = 本会话内「必须让 Agent 理解」的文档/规则总 token（例如 5 份核心文档 ≈ 30 万）。
- **C** = 当前会话已产生的**对话历史**总 token（你的提问 + Agent 回复等）。

每轮请求的 Cache Read ≈ F + C。新开会话时至少重新加载一次文档，成本约 F。

### 平衡法则

- **当 C ≥ F**：多聊一轮的 cache 成本 (F+C) 已高于「新开一场只加载文档」的成本 (F)。此时收尾当前会话，新任务在新会话里做并重新 @ 文档更省。
- **当 C < F**：对话历史还不大，可继续在当前会话；若换任务或换文档集，再开新会话。

### 文档多时的推荐做法

1. **做总纲/索引文档**：每份只写主题、结论、关键数字、对应详情的路径。Agent「充分理解」时优先基于总纲 + 当前任务相关的 1～2 份详文。
2. **单会话只 @ 当前任务需要的文档**：不在一场会话里 @ 全量文档。
3. **按任务边界切会话**：同一任务、同一批文档、连续推进 → 可继续；任务结束 / 换主题 / 换文档集 → 新会话并重新 @ 本任务所需文档。
4. **用数据校准**：若单次请求 Cache Read 经常 > 1M 且多为重复历史，说明会话偏长，可更果断按任务切会话并减少单会话 @ 的文档量。
