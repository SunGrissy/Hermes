...
name: plain-speak
description: >-
  Use when user says '白话说', '说人话', '解释一下', '用大白话讲', 'plain',
  'grok', or wants any content rewritten so a smart 12-year-old can understand it.
---
...
```
User: 白话说一下什么是贝叶斯推断
Assistant: [用大白话重写，零术语，聪明的12岁孩子能复述]
```
...
- *类比* — 找结构对得上的日常经验。好类比承重（去掉它文章塌），多层（挖一层还像），自明（不需要解释类比本身）
...
形式自由。输出是一篇从第一行流到最后一行的连贯文章。全文只有文件标题，正文无子标题。
...
> 原始设计：[ljg-plain](https://github.com/lijigang/ljg-skills) by lijigang
