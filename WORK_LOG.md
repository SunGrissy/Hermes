...
**内容**:
- `会议材料/驯化野生硅基伙伴_可交互版.html`：补充内容编辑相关交互能力，保留原分享稿页面演示、搜索、总览和翻页功能。

**测试**:
- 静态检查确认文件存在，核心关键字 `search`、`overview`、`slide`、`contenteditable` 存在。
- `git diff --check` 通过。

---

## [2026-05-04] - 会议材料：可编辑互动版分享稿内容结构更新

**状态**: 验收通过（本提交）

**内容**:
- `会议材料/驯化野生硅基伙伴_可编辑互动版.html`：将前半段结构调整为“四个真实困惑 → 解惑路径 → Bonus 造锤子”，对齐分享开场逻辑。
- 保留原可编辑互动能力：正文可编辑、保存状态、导出保存版、搜索、导航与页面演示交互。

**测试**:
- 静态检查确认文件存在，核心交互关键字 `contenteditable`、`localStorage`、`drop` 存在。
- Diff 敏感信息扫描未命中。

---

## [2026-05-04] - 会议材料：驯化野生硅基伙伴（渐进披露版）PPTX 入库

**状态**: 已完成（本提交）

**内容**:
- `会议材料/驯化野生硅基伙伴_渐进披露版.pptx`：版本更新并纳入根仓跟踪。
- `会议材料/驯化野生硅基伙伴_渐进披露版.md`：MarkItDown 转换自 PPTX，脚本修正 33 处页码使其与幻灯片真实序号一致。

**测试**: 转换后逐页核对 `<!-- Slide number -->` 与正文末尾页码，共 33 页对应无误。
...
**内容**:
- 策划周计划弹窗：分类中文、负责人/阶段下拉、目标周口径说明、Esc 正确关闭（避免全局 Esc 仅去 active 导致错位）。
- 运营矩阵 `import-ops-lead-window`：`target_week` 按标签所在矩阵 ISO 周写入、扩大扫描窗口、打开周计划时刷新已有同步行标题与负责人。
- 子模块内相关 backend / `data-service.js` / 测试等一并提交。
...
**内容**:
- `normalize_excel_column_aliases`：`消耗`/`展示`/`上线日期`/`新增`/`新增成本`、`广告名称`、`投放版位` 等与标准列对齐；四象限图缺 `当前总消耗` 时兜底。
- 修复 v2 分支中 `当前总消耗` 补全逻辑误放在 `st.stop()` 后导致永不执行的问题。
- 版本号与 `README.md` 数据契约 §2.5 别名表同步；`utils.LAST_UPDATE_TIME` 更新。
...
**内容**:
- `pm-system/quick_start.bat`：命令行 `1`-`8`（单项重启 / 全关 / 全启）；无参数启动后展示编号服务列表与中文提示，循环输入命令；`Q` 仅关闭本启动器窗口（服务保留）
...
## [2026-04-13] - 本地改动分批提交：子模块与根目录材料/工具
...
**内容**:
- 新增 `boss-design-council` Skill：5 位认知科学视角的智囊围绕 Boss 方案讨论
  - 注意力设计师（DAN/VAN、工作记忆、焦点引导）
  - 预期工程师（PEL、VDAC、Near-miss、信号→赌注→揭晓）
  - 情绪建筑师（峰终定律、蓄力-释放、着急 > 拖沓）
  - 玩家代言人（归因理论、损失厌恶、公平感、向前读）
  - 节奏指挥（系统 1/2 交替、时间操控、心流通道）
- 设计哲学：苏格拉底式提问，不做裁判不给结论，答案由策划自己给出
- 与预审三道门互补：智囊团帮内化标准，三道门做终检
- Superpowers 格式，可独立分发给同事安装
