...
**内容**:
- 新建 `MEETING_SYSTEM.md`：工作室全局会议体系架构文档
- 双轴设计：事轴（版本仪式 / Feature 质控 / 管线运营 / 经营决策）+ 人轴（全员触达 / 职能组 / 个人发展）
- Feature 级质控：系统设计审核、内容方案审核、战斗 WHAT+HOW 审核、体验走查均为 Feature 维度 event-driven
- 工具门禁：DoR / DoD / 滞留预警 / 返工追踪，PmSystem 强制
- 会前清单制替代传统会议纪律
- 灵活替代全员大会：月度广播(异步) + 走动式管理(周) + 重大节点大会(半年)
- WHAT 层月度同步：策划 + 数据分析(杨琳) + 数值(刘洋/徐一卜) 联合
- 项目组级跨职能分享会（轮值主办组）
- 职能组每日站会 + 1:1 双机制配置
- 初级管理者指引（三问模型、会议操作手册）
