...
> 创建日期：2026-03-03
> 状态：**基本完成**（P-1~P-4, R-1~R-4 全部完成，G-1 受 Free 计划限制跳过，G-2/G-3 已验证）
...
| 编号 | 任务 | 负责人 | 依赖 | 状态 |
|:---:|------|:------:|:---:|:----:|
| P-1 | 密钥外置化改造（pm-system config.py） | AgentSync | 无 | **已完成** |
...
| 编号 | 任务 | 依赖 | 状态 |
|:---:|------|:---:|:----:|
| G-1 | 6 个仓库配置 main 分支保护规则（Web UI） | R-4, P-4 | **跳过**（Free 计划不支持） |
| G-2 | 验证：Cursor 云端 clone + 子模块初始化 | R-4 | **已验证**（API 确认内容完整） |
| G-3 | 验证：dev_mode 启动后端应用 | G-2, P-1 | 待实际 clone 验证 |
