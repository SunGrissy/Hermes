...
**文档版本**：0.6  
**更新日期**：2026-04-12  
**读者**：产品经理、项目经理、前后端/服务端工程师、测试、数据分析；**接收实施的同学以本文档为入口**（含仓库约定与对接方式）。  
...
本产品的设计决策、依赖的方法论与实现说明**均以本 Git 仓库（MyAgents）内文件为唯一事实源**；不依赖口传或外部散落链接。若口头说明与本文档或仓库内容不一致，**以本仓库为准**。
...
| 虚拟用户行为模拟（有档案时自动增强，无则降级为纯引擎审查） | 虚拟用户档案创建（独立流程，见 `virtual-player-creation-guide.md`） |
...
| 0.5 | 2026-04-11 | 文档目的下增加「文档与仓库约定（实施对接）」；依赖索引、对接制作人、双 Git 指引；读者含实施方 |
| 0.6 | 2026-04-12 | §1.3 范围边界补充虚拟用户行为模拟能力；§5.4 补充虚拟用户集成引用 |
