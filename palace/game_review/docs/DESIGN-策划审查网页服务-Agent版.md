...
**文档版本**：0.5（与人类版 v0.6 对齐）  
**更新日期**：2026-04-12  
**读者**：在本仓库内实现/修改该功能的 AI Agent 与工程师  

**产品权威**：范围、分期、验收、数据留存与审计、**仓库与实施对接约定**以 [`DESIGN-策划审查网页服务-人类版.md`](DESIGN-策划审查网页服务-人类版.md) **v0.6** 为准（尤其 **文档目的 · 文档与仓库约定**、**§2.6、§3.1、§4.3、§5.7**）。本文档为实现侧约束与接口草案；**冲突时以人类版为准并回写修订**。
...
**虚拟用户降级**：审查引擎的虚拟用户行为测试为可选增强。网页服务**无需特殊处理**——引擎内部检测 `virtual-players/` 目录，有档案则输出含虚拟用户章节的报告，无则跳过。网页侧仅需确保终稿 Markdown 渲染兼容该章节（标准 Markdown 表格，无额外依赖）。
...
- [ ] 关键失败路径触发维护通知。
- [ ] `virtual-players/` 目录不存在时，审查报告正常生成（无虚拟用户章节）；目录存在时，报告含虚拟用户行为测试章节且渲染正常。
...
| 0.5 | 2026-04-12 | 与人类版 **v0.6** 对齐：§3 补充虚拟用户集成路径、降级说明；§12 补充虚拟用户降级测试项 |
