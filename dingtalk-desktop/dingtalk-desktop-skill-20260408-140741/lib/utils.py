...
DEFAULT_MY_UID = os.environ.get('DINGTALK_MY_UID', '')


def canonical_p2p_cid(cid, my_uid):
    """
    单聊会话 id 规范为「己方 uid:对方 uid」。
    若 cid 为「对方:己方」则返回规范键；非单聊或无法识别时返回 None。
    """
    if not cid or not my_uid:
        return None
    if ':' not in cid or cid.startswith('cid'):
        return None
    parts = cid.split(':', 1)
    if len(parts) != 2:
        return None
...
def _parse_seen_ts(s):
    """contacts.json 中 first_seen / last_seen 混用 ISO-T 与 'YYYY-MM-DD HH:MM:SS'。"""
...
def _earlier_seen(a, b):
    if not a:
        return b or ''
    if not b:
        return a
    ta, tb = _parse_seen_ts(a), _parse_seen_ts(b)
    if ta and tb:
...
def merge_p2p_entries(into, fr, my_uid=None):
    """将 fr 合并进 into（同一单聊的两条互反记录）。into 会被原地修改。"""
    mu = str(my_uid if my_uid is not None else DEFAULT_MY_UID)

    into['first_seen'] = _earlier_seen(
        into.get('first_seen') or '', fr.get('first_seen') or '')
    into['last_seen'] = _later_seen(
        into.get('last_seen') or '', fr.get('last_seen') or '')

    into['msg_count'] = into.get('msg_count', 0) + fr.get('msg_count', 0)

    fr_uid_bad = str(fr.get('uid') or '') == mu
    if fr_uid_bad:
        return into

    pr, sr = into.get('name_resolved'), fr.get('name_resolved')
    pn, sn = (into.get('name') or ''), (fr.get('name') or '')
    if sr and not pr:
        into['name'] = sn
        into['name_resolved'] = True
    elif pr and not sr:
        pass
    elif sn and (not pn or len(sn) > len(pn)):
        into['name'] = sn
        into['name_resolved'] = bool(sr or into.get('name_resolved'))
    return into


ADV_SEARCH_URL = (
    'app://desktop.dingtalk.com/web_content/advancedSearch.html'
    '?hideInput=true&isBigScreen=true&edition=enterprise'
    '&newSearch=1&newSearchIcon=1#/MultiSearch'
)

CT_NAMES = {
    1: '文本', 101: '图文', 203: '图片', 300: '工作汇报',
    302: '语音', 305: '视频', 501: '大文件', 502: '文件', 503: '文件夹',
    1200: 'Markdown', 2001: '文件',
    2950: '互动卡片', 3100: '富文本/@',
}
...
class ContactsDB:
    ...
    @classmethod
    def _dedupe_p2p(cls):
        """合并互反单聊键，迁移 api_cid → 正式 key，清除 api_cid 字段。"""
        db = cls._cache
        if not db:
            return
        p2p = db.setdefault('p2p', {})

        for cid in list(p2p.keys()):
            entry = p2p[cid]
            api = entry.pop('api_cid', None)
            if api and api != cid:
                src = p2p.pop(cid)
                src['cid'] = api
                if api in p2p:
                    merge_p2p_entries(p2p[api], src)
                else:
                    p2p[api] = src
                cls._dirty = True
        ...
        for cid in list(p2p.keys()):
            ...
            if pair in seen:
                existing_cid = seen[pair]
                existing = p2p[existing_cid]
                current = p2p.pop(cid)
                if current.get('msg_count', 0) > existing.get('msg_count', 0):
                    p2p.pop(existing_cid)
                    merge_p2p_entries(current, existing)
                    current['cid'] = cid
                    p2p[cid] = current
                    seen[pair] = cid
                else:
                    merge_p2p_entries(existing, current)
                cls._dirty = True
            else:
                seen[pair] = cid

        for entry in p2p.values():
            if 'api_cid' in entry:
                del entry['api_cid']
                cls._dirty = True
    ...
    def _get(cls, cid):
        ...
        if entry:
            return entry
        if section == 'p2p' and ':' in cid:
            parts = cid.split(':', 1)
            return db[section].get(f'{parts[1]}:{parts[0]}')
        return None
    ...
    def update(cls, cid, sender_uid, my_uid, is_self=False):
        ...
        with cls._lock:
            ...
            if not entry and section == 'p2p' and ':' in cid:
                parts = cid.split(':', 1)
                reverse = f'{parts[1]}:{parts[0]}'
                entry = db[section].get(reverse)
                if entry:
                    del db[section][reverse]
                    entry['cid'] = cid
            ...
            if is_p2p:
                parts = cid.split(':')
                mu = str(my_uid)
                other_uid = parts[1] if parts[0] == mu else parts[0]
    ...
    @classmethod
    def search_by_exact_display_name(cls, name):
        """显示姓名与 name（strip 后）完全一致时命中；不按 CID/UID 子串匹配。"""
        q = (name or '').strip()
        if not q:
            return []
        with cls._lock:
    ...
    def set_name(cls, cid, name):
        ...
        with cls._lock:
            ...
            if not entry and section == 'p2p' and ':' in cid:
                parts = cid.split(':', 1)
                entry = db[section].get(f'{parts[1]}:{parts[0]}')
            if not entry:
                return False
