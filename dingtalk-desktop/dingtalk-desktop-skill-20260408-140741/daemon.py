...
_IMAGE_CACHE_DIR = os.path.join(DATA_DIR, 'download', 'image')
...
_FILE_CACHE_DIR = os.path.join(DATA_DIR, 'download', 'file')
...
def _try_download_im_file(f_id, s_id, f_name, f_type, media_type='file'):
    """Try to download an IM file using storage API. Returns local path or ''."""
    if not f_id or not s_id:
        return ''
    cache_dir = _IMAGE_CACHE_DIR if media_type == 'image' else _FILE_CACHE_DIR
    safe_name = f_name.replace('/', '_').replace('\\', '_').replace(':', '_')
    if not safe_name:
        safe_name = f'{f_id}.{f_type}' if f_type else str(f_id)
    local_path = os.path.join(cache_dir, f'{f_id}_{safe_name}')
    if os.path.exists(local_path) and os.path.getsize(local_path) > 0:
        return local_path
...
if (!libcef) {
    ...
    rpc.exports = {
        setNextSavePath: function(path) { _nextSavePath = path; return 'ok'; },
    ...
    var _nextSavePath = null;
    var _defaultSaveDir = '""" + SILENT_DL_DIR.replace('\\', '\\\\') + """';
    try {
        ...
        Interceptor.replace(pGetSave, new NativeCallback(function(lpofn) {
            try {
                ...
                var savePath;
                if (_nextSavePath) {
                    savePath = _nextSavePath;
                    _nextSavePath = null;
                } else {
                    var safeName = proposed || 'download.bin';
                    savePath = _defaultSaveDir + '\\\\' + safeName;
                }
    ...
    def _loop(self):
        while self._running:
            ...
            for cid, info in expired:
                self._process_cid(cid, info['records'])

            time.sleep(0.3)

    def _process_cid(self, cid, records):
        ...
        Media (images/files) are NOT downloaded here — only metadata is extracted.
        Gateway downloads media on-demand after rule matching."""
...
class FridaDaemon:
    ...
    def _verify_jsapi_browser(self, bid):
        ...
        if reports.get('chk', False) is True:
            return True
        # Fallback: app:// pages may block fetch to localhost; use hash-based check
        try:
            import base64 as _b64
            marker = '__JSAPI_CHK_'
            hash_js = (
                "(function(){"
                "var r=typeof dingtalk!==\"undefined\"&&!!dingtalk.message"
                "&&typeof dingtalk.message.listMessage===\"function\";"
                "location.hash='" + marker + "'+r;"
                "})()"
            )
            self._cef_script.exports_sync.exec_js(bid, hash_js)
            time.sleep(0.5)
            url = self._cef_script.exports_sync.get_browser_url(bid)
            if url and marker in url:
                val = url.split(marker, 1)[1]
                return val == 'true'
        except Exception:
            pass
        return False
    ...
    def fetch_history(self, cid, count=20, before=None, after=None, timeout=30,
        ...
        download_media: 是否自动下载图片/文件（False 时只提取元数据）
        """
        ...
        with self._fetch_lock:
            ...
            messages = self._format_jsapi_messages(msgs_raw, download_media=download_media)
            self._fetch_count += 1
    ...
    def _format_jsapi_messages(self, msgs_raw, download_media=True):
        """解析 JSAPI dingtalk.message.listMessage 返回的消息列表.
        download_media=False 时只提取元数据（URL/file_info），不实际下载。
        用于事件推送场景——由 Gateway 规则匹配后按需下载。"""
        ...
        for m in msgs_raw:
            ...
            # ct=203: pure image — resolve local cache, then JSAPI / CDN fallback
            if ct == 203:
                img_url = m.get('img', '')
                att_ext = {}
                if raw:
                ...
                if att_ext:
                    img_fi = {}
                    for k in ('f_id', 's_id', 'f_name', 'f_type'):
                        if att_ext.get(k):
                            img_fi[k] = att_ext[k]
                    if img_fi.get('f_id') and img_fi.get('s_id'):
                        entry['image_file_info'] = img_fi
                ...
                if local_path and not os.path.isfile(local_path):
                    ...
                    local_path = ''
                if download_media:
                    # JSAPI download_im_file first (preferred for IM images)
                    if not local_path and att_ext:
                        ...
                        if f_id_203 and s_id_203:
                            local_path = _try_download_im_file(f_id_203, s_id_203, f_name_203, f_type_203, media_type='image')
                    # CDN fallback
                    ...
                    if not local_path and raw:
                        try:
                            ...
                            pass
                if not local_path and download_media:
            ...
            # ct=501/502/503: File messages — extract metadata, download only if requested
            if ct in (501, 502, 503) and raw:
                try:
                    ...
                    if atts:
                        ...
                        if download_media and s_id and f_id:
                            try:
                                dl_result = self.download_im_file(s_id, f_id, f_name, timeout=20)
                                if dl_result.get('success') and dl_result.get('path'):
                                    dl_path = dl_result['path']
                                    if os.path.isfile(dl_path) and os.path.getsize(dl_path) > 0:
                                        entry['file_local_path'] = dl_path
                                        log(f'[ct={ct}] auto-downloaded: {f_name} → {dl_path}')
                                    elif dl_result.get('downloading'):
                                        log(f'[ct={ct}] download in progress: {f_name}')
                                    else:
                                        log(f'[ct={ct}] download returned path but file missing: {dl_path}')
                                else:
                                    log(f'[ct={ct}] download failed for {f_name}: {dl_result.get("error", "?")}')
                            except Exception as dl_e:
                                log(f'[ct={ct}] download exception for {f_name}: {dl_e}')
                except Exception as e:
    ...
    def download_im_file(self, space_id, dentry_id, file_name='', timeout=20, media_type='file'):
        ...
        local_path = os.path.join(cache_dir, f'{dentry_id}_{safe_name}')
        if os.path.exists(local_path) and os.path.getsize(local_path) > 0:
            return {'success': True, 'path': local_path, 'cached': True}

        if not self.is_attached or not self._ensure_b1():
            local = _try_download_im_file(dentry_id, space_id, file_name, '', media_type=media_type)
            if local:
                return {'success': True, 'path': local, 'method': 'storage_api'}
            return {'success': False, 'error': 'DingTalk not attached'}

        def _copy_to_cache(src_path):
            """Copy file to cache_dir if not already there."""
            if not src_path or not os.path.isfile(src_path):
                return src_path
            if os.path.dirname(os.path.abspath(src_path)) == os.path.abspath(cache_dir):
                return src_path
            try:
                shutil.copy2(src_path, local_path)
                log(f'[file-dl] Copied to cache: {src_path} → {local_path}')
                return local_path
            except Exception as e:
                log(f'[file-dl] Failed to copy to cache: {e}')
                return src_path

        # Step 1: check listAllTask for existing download
        existing = self._find_task_file(dentry_id)
        if existing and os.path.isfile(existing):
            final = _copy_to_cache(existing)
            log(f'[file-dl] Found in listAllTask: {existing}')
            return {'success': True, 'path': final, 'method': 'listAllTask'}

        # Step 2: createTask — tell Hook to save directly to cache path
        try:
            self._cef_script.exports_sync.set_next_save_path(local_path)
        except Exception as e:
            log(f'[file-dl] setNextSavePath failed: {e}')
        ...
        if isinstance(result, dict) and result.get('taskId'):
            # createTask succeeded — Hook should save directly to local_path (cache dir)
            time.sleep(2)
            if os.path.isfile(local_path) and os.path.getsize(local_path) > 0:
                log(f'[file-dl] Downloaded directly to cache: {local_path}')
                return {'success': True, 'path': local_path, 'method': 'createTask'}

            # Fallback: Hook might not have fired, check listAllTask
            actual_path = self._find_task_file(dentry_id)
            if actual_path and os.path.isfile(actual_path):
                final = _copy_to_cache(actual_path)
                log(f'[file-dl] Found via listAllTask: {actual_path}')
                return {'success': True, 'path': final, 'method': 'createTask'}
            ...
            return {
                'success': True,
                'path': local_path if os.path.isfile(local_path) else (actual_path or ''),
                'method': 'createTask',
    ...
    def _ensure_b1(self):
        ...
        if isinstance(url, str) and 'advancedSearch' in url and has_api:
            if not self._b1_original_url:
                self._b1_original_url = url
                log(f'B1 原始 URL 已保存: {url[:120]}')
            return True

        restore_url = self._b1_original_url or ADV_SEARCH_URL
        log(f'B1 不在 advancedSearch，尝试恢复 (using {"saved" if self._b1_original_url else "fallback"} URL)...')
        self._cef_script.exports_sync.load_url(1, restore_url)
        time.sleep(4)
    ...
    def fetch_report_content(self, url, timeout=30, wait=15, selectors=None, custom_js=None):
        """通过 CEF loadUrl 打开页面并提取正文内容

        优先用 CSS 选择器定位正文容器，避免提取导航栏、侧栏等噪音。
        多选择器逐级 fallback，最终 fallback 到 body.innerText。

        页面加载使用轮询检测：每 0.5s 检查目标选择器是否已渲染出内容，
        就绪后立即提取，不再固定等待。大多数报告页 2~3s 即可就绪。

        Args:
            url: 要加载的页面 URL
            timeout: 等待 beacon 响应的超时秒数
            wait: 页面加载最大等待秒数（轮询上限，默认15）
        ...
        with self._fetch_lock:
            ...
            # 轮询等待页面就绪，而非固定 sleep
            sels_js_poll = ','.join(f"'{s}'" for s in selectors)
            poll_js = (
                "(function(){var P=" + str(BEACON_PORT) + ";"
                "var sels=[" + sels_js_poll + "];"
                "var ready=false,method='none',len=0;"
                "for(var i=0;i<sels.length;i++){"
                "try{var e=document.querySelector(sels[i]);"
                "if(e&&e.innerText&&e.innerText.trim().length>20){"
                "ready=true;method=sels[i];len=e.innerText.trim().length;break;}"
                "}catch(x){}}"
                "if(!ready&&document.body&&document.body.innerText&&"
                "document.body.innerText.trim().length>100){"
                "ready=true;method='body';len=document.body.innerText.trim().length;}"
                "fetch('http://127.0.0.1:'+P+'/r?l=_poll',"
                "{method:'POST',body:JSON.stringify({ready:ready,method:method,"
                "len:len,rs:document.readyState,url:location.href}),"
                "mode:'no-cors'}).catch(function(){});})()"
            )
            page_ready = False
            poll_interval = 0.5
            time.sleep(1)  # 给页面最少 1s 启动时间
            for _ in range(int((wait - 1) / poll_interval)):
                self._beacon.clear()
                try:
                    self._cef_script.exports_sync.exec_js(1, poll_js)
                except Exception:
                    pass
                time.sleep(poll_interval)
                reports = self._beacon.get_reports()
                poll_data = reports.get('_poll')
                if isinstance(poll_data, dict) and poll_data.get('ready'):
                    page_ready = True
                    break

            if not page_ready:
                log(f'[fetch_report_content] page not ready after {wait}s, proceeding anyway')
    ...
    def fetch_reports_paginated(self, count=10, before=None, after=None,
        ...
        for m in msgs:
            ...
            if not report_url:
                continue
            try:
                cr = self.fetch_report_content(report_url, wait=12)
                if cr.get('success'):
                    full_text = cr.get('content', '')
                    if len(full_text) > len(text):
                        m['text'] = full_text
                        m['reloaded'] = True
                        reloaded_count += 1
                        log(f'[fetch_reports] reloaded truncated report: {len(text)}→{len(full_text)} chars')
            except Exception as e:
                log(f'[fetch_reports] reload failed: {e}')
        ...
        if reloaded_count:
            result['reloaded'] = reloaded_count
        if timed_out:
...
class DaemonHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif parsed.path == '/enrich-messages':
            msgs = body.get('messages', [])
            if not isinstance(msgs, list):
                self._json_response({'error': 'messages must be a list'}, 400)
                return
            enriched = []
            for m in msgs:
                ct = m.get('content_type', 0)
                updated = dict(m)
                # ct=203: JSAPI download_im_file preferred, CDN fallback
                if ct == 203 and not m.get('image_local_path'):
                    img_fi = m.get('image_file_info', {})
                    s_id_img = img_fi.get('s_id', '')
                    f_id_img = img_fi.get('f_id', '')
                    f_name_img = img_fi.get('f_name', '')
                    if s_id_img and f_id_img:
                        try:
                            dl = _daemon.download_im_file(s_id_img, f_id_img, f_name_img, timeout=20, media_type='image')
                            if dl.get('success') and dl.get('path'):
                                dp = dl['path']
                                if os.path.isfile(dp) and os.path.getsize(dp) > 0:
                                    updated['image_local_path'] = dp
                                    log(f'[enrich] ct=203 JSAPI downloaded: {f_name_img} → {dp}')
                        except Exception as e:
                            log(f'[enrich] ct=203 JSAPI error for {f_name_img}: {e}')
                    if not updated.get('image_local_path'):
                        img_url = m.get('image_url', '')
                        if img_url and img_url.startswith('https://'):
                            lp = _download_cdn_image(img_url)
                            if lp:
                                updated['image_local_path'] = lp
                # ct=3100: rich text images — CDN only (no per-image f_id)
                if ct == 3100 and not m.get('image_local_path'):
                    img_url = m.get('image_url', '')
                    img_urls = m.get('image_urls', [])
                    urls_to_download = img_urls if img_urls else ([img_url] if img_url else [])
                    if urls_to_download:
                        local_paths = []
                        for url in urls_to_download:
                            if url.startswith('https://'):
                                lp = _download_cdn_image(url)
                                if lp:
                                    local_paths.append(lp)
                        if local_paths:
                            updated['image_local_path'] = local_paths[0]
                            if len(local_paths) > 1:
                                updated['image_local_paths'] = local_paths
                # ct=501/502/503: download file via JSAPI
                if ct in (501, 502, 503) and not m.get('file_local_path'):
                    fi = m.get('file_info', {})
                    s_id = fi.get('s_id', '')
                    f_id = fi.get('f_id', '')
                    f_name = fi.get('f_name', '')
                    if s_id and f_id:
                        try:
                            dl = _daemon.download_im_file(s_id, f_id, f_name, timeout=20)
                            if dl.get('success') and dl.get('path'):
                                dp = dl['path']
                                if os.path.isfile(dp) and os.path.getsize(dp) > 0:
                                    updated['file_local_path'] = dp
                                    log(f'[enrich] downloaded file: {f_name} → {dp}')
                        except Exception as e:
                            log(f'[enrich] download error for {f_name}: {e}')
                enriched.append(updated)
            self._json_response({'success': True, 'messages': enriched})
