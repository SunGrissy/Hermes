...
_ROLE_KEYWORDS = {
    ...
    '主策划':   ['主策划', '策划总监', '主策', 'lead designer', '策划负责人', 'game director'],
    '系统策划': ['系统策划', '系统', '数值', 'sys'],
...
def _build_prompt(resume_text: str, role: str, checklist: str,
    ...
    common_thinking = (
        '【通用思维考察（所有岗位适用）】'
        '在评估时额外关注以下 3 个维度：'
        '1. AI与进化力：会用(提到工具名) < 能用(辅助产出) < 改造(重构工作流/可量化提效)；'
        '2. 深度思考：能从数据/现象追到根因，有trade-off分析、复盘或方法论输出；'
        '3. 第一性原理：从底层逻辑/用户需求本质出发推导方案，并能质疑现有做法。'
        '无证据不扣分，有明确证据应写入亮点。'
    )
    ...
    if is_fresh:
        ...
        system = (
            '你是一位游戏公司的人力资源专家，擅长策划管理岗简历初筛。\n'
            '本次筛选的目标定级为 **L4 主策划**，请严格按以下标准评估。\n\n'
            '【L4 核心能力要求】\n'
            '1. 游戏整体方向定义：独立制定或深度参与产品玩法方向、版本规划、长线节奏\n'
            '2. 跨系统顶层架构：能从全局规划系统关系与边界，超越单系统耦合层面\n'
            '3. 策划团队管理：带过策划团队，有标准制定、评审机制或方法论输出\n'
            '4. 跨职能协同决策：与主美/主程/运营等实质协同，参与关键体验与技术决策\n'
            '5. 品类与商业理解：对目标用户付费心理、生命周期和大R运营有体系化认知\n\n'
            '【硬性门槛】\n'
            '- 至少 1 款已上线产品中，具备稳定运营期的事实主策经历（不卡title，但必须实际承担主策职责）\n\n'
            '【L4 vs L3 判定规则】\n'
            '- 有方向定义 + 架构规划 + 团队管理证据 -> 可能达到 L4\n'
            '- 仅有多系统耦合/框架扩展，无全局方向或带人经验 -> L3 水平\n'
            '- 仅体现单系统独立全流程 -> L2 水平\n'
            '- 以"参与/协助"为主、无独立主导经验 -> L1 水平\n'
            '- L3 扎实但 L4 证据不足 -> 结论给"待定"，注明"L3扎实、L4待面试验证"\n\n'
            '【禁止臆测】简历中没有明确方向定义、架构规划、团队管理等表述时，'
            '不得判定为"体现 L4 思维"。\n'
            + common_thinking +
            '输出格式严格遵守要求，不要添加额外说明。'
        )
        ...
        output_hint = (
            ...
            'L4评估：[简历是否体现L4主策核心能力（方向定义/全局架构/团队管理/跨职能决策），'
            '<=40字，无证据则写"简历未体现L4主策证据"]\n'
            '定级判断：[L4可能 / L3扎实L4待验 / L3 / L2+ / 不达标]\n'
            '理由：[1-2句话，聚焦最关键的依据]\n'
        ...
        system = (
            '你是一位游戏公司的人力资源专家，擅长战斗策划岗简历初筛。\n'
            '本次筛选目标为 **L3 战斗策划**，请按下列标准评估。\n\n'
            '【L3 核心能力要求】\n'
            '1. 战斗系统化能力：不止做单点技能，能构建或维护战斗规则、技能循环、关卡节奏体系\n'
            '2. 体验与数值平衡：能在手感、反馈、节奏与数值之间做取舍并落地\n'
            '3. 全流程落地：有文档/配置/联调/验收闭环，而非仅参与讨论\n'
            '4. 跨职能协作：与程序、动画、特效有效协作推进上线\n'
            '5. 复盘迭代：有基于数据和玩家反馈优化战斗体验的证据\n\n'
            '【L3 vs L2 判定规则】\n'
            '- 有体系化战斗框架和跨模块协同证据 -> 可能达到 L3\n'
            '- 仅体现单点技能或关卡执行，缺乏体系化能力 -> L2 水平\n'
            '- 以参与/协助为主、无独立主导闭环 -> L1 水平\n'
            '- L2 扎实但 L3 证据不足 -> 结论给"待定"\n'
            '- 简历无法体现 L2 以上能力 -> 结论给"不通过"\n\n'
            '【禁止臆测】简历中没有明确战斗框架、闭环落地、复盘优化等表述时，'
            '不得判定为"体现 L3 战斗能力"。\n'
            + common_thinking +
            '输出格式严格遵守要求，不要添加额外说明。'
        )
        ...
        output_hint = (
            ...
            '结论：[通过 / 待定 / 不通过]\n'
            '核心判定：[一句话<=20字，说明该结论的决定性原因]\n'
            'L3评估：[简历是否体现L3战斗策划核心能力（体系化战斗框架/全流程落地/跨职能协作/复盘迭代），'
            '<=40字，无证据则写"简历未体现L3战斗体系能力"]\n'
        ...
        system = (
            '你是一位游戏公司的人力资源专家，擅长运营策划岗简历初筛。\n'
            '本次筛选目标为 **L3 运营策划**，请按下列标准评估。\n\n'
            '【L3 核心能力要求】\n'
            ...
            '【L3 vs L2 判定规则】\n'
            '- 有活动全流程Owner + 数据归因与优化证据 -> 可能达到 L3\n'
            '- 仅体现执行配置/日常运营，缺少独立Owner与复盘闭环 -> L2 水平\n'
            '- 以参与/协助为主、无独立主导 -> L1 水平\n'
            '- L2 扎实但 L3 证据不足 -> 结论给"待定"\n'
            '- 简历无法体现 L2 以上能力 -> 结论给"不通过"\n\n'
            '【禁止臆测】简历中没有明确目标设定、数据归因、活动复盘等表述时，'
            '不得判定为"体现 L3 运营能力"。\n'
            + common_thinking +
            '输出格式严格遵守要求，不要添加额外说明。'
        )
        ...
        output_hint = (
            ...
            '结论：[通过 / 待定 / 不通过]\n'
            '核心判定：[一句话<=20字，说明该结论的决定性原因]\n'
            'L3评估：[简历是否体现L3运营策划核心能力（活动Owner/数据归因/商业化理解/协同与沉淀），'
            '<=40字，无证据则写"简历未体现L3运营闭环能力"]\n'
            '定级判断：[L3可能 / L2扎实L3待验 / L2 / L1+ / 不达标]\n'
            '理由：[1-2句话，聚焦最关键的依据]\n'
            '亮点：[命中的优先信号，逗号分隔，没有则写"无"]\n'
            '红线：[触发的红线，逗号分隔，没有则写"无"]\n'
        ...
        system = (
            '你是一位游戏公司的人力资源专家，正在做策划岗位简历初筛。\n'
            f'当前候选岗位为：{role}。\n'
            '请优先依据岗位清单判断硬性条件、红线和优先信号，不要套用其他岗位标准。\n'
            '若信息不足，结论给"待定"并明确初试核实点。\n'
            + common_thinking +
            '输出格式严格遵守要求，不要添加额外说明。'
        )
        checklist_section = (
        ...
        output_hint = (
            ...
            '结论：[通过 / 待定 / 不通过]\n'
            '核心判定：[一句话<=20字，说明该结论的决定性原因]\n'
            'L3评估：[简历是否体现目标岗位的中高级能力，<=40字，无证据则写"简历关键证据不足"]\n'
            '定级判断：[待定]\n'
            '理由：[1-2句话，聚焦最关键的依据]\n'
            '亮点：[命中的优先信号，逗号分隔，没有则写"无"]\n'
            '红线：[触发的红线，逗号分隔，没有则写"无"]\n'
...
def _format_reply(file_name: str, role: str, parsed: dict,
    ...
    ctx = {
        ...
        'l4_assess':  parsed.get('l4_assess', ''),
